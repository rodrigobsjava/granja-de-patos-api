...
public class PatoService {
	...
	public Pato findById(UUID id) {
		return patoRepository.findById(id).orElseThrow(() -> new NotFoundException("Pato não encontrado"));
	}
	...
	public void delete(UUID id) {
		...
		if (pato.isVendido()) {
			throw new BusinessException("Não é possível excluir um pato já vendido");
		}
		patoRepository.delete(pato);
	}

	public PatoResponseDTO toResponseDTO(Pato pato) {
		if (pato == null)
			return null;

		return PatoResponseDTO.builder().id(pato.getId()).nome(pato.getNome())
				.pataMaeId(pato.getPataMae() != null ? pato.getPataMae().getId() : null).vendido(pato.isVendido())
				.preco(pato.getPreco()).build();
	}
}
